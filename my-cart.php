...
if (isset($_POST['ordersubmit'])) {
    if (strlen($_SESSION['login']) == 0) {   
        ...
        foreach ($value as $qty => $val34) {
            ...
            $query = mysqli_query($con, "SELECT productQuantity FROM products WHERE id='$qty'");
            $row = mysqli_fetch_array($query);
            $newQuantity = $row['productQuantity'] - $val34;

            // Make sure not to set negative quantity
            if ($newQuantity >= 0) {
                mysqli_query($con, "UPDATE products SET productQuantity='$newQuantity' WHERE id='$qty'");
            }
        }
...
<td class="cart-product-sub-total"><span class="cart-sub-total-price"><?php echo "BDT
